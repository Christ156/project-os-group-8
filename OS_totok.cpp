 ...
 */
 ...
 /* Round Robin Scheduling Algorithm (Accurate Queue Implementation)
 * Menjalankan proses sesuai urutan kedatangan menggunakan Ready Queue.
 * Jika proses baru datang bersamaan dengan selesainya kuantum proses lama,
 * proses baru akan diprioritaskan masuk antrean lebih dulu.
 */
...
void roundRobin(struct Process p[], int n, int tq) {
    ...
    int remainingBT[n];
    bool inQueue[n]; // Untuk menandai apakah proses sudah masuk antrean
    int queue[n * 10]; // Antrean sederhana (ukuran diperbesar untuk keamanan)
    int head = 0, tail = 0;
    ...
    // Masukkan semua proses yang tiba di waktu 0 ke dalam antrean
    for (int i = 0; i < n; i++) {
    ...
    while (completed < n) {
        // Jika antrean kosong, tandai IDLE
        if (head == tail) {
            printf("| IDLE (%d) ", currentTime + 1);
            currentTime++;
            // Cek apakah ada proses yang datang saat idle
            for (int i = 0; i < n; i++) {
        ...
        // Simpan waktu awal untuk pengecekan arrival nanti
        ...
        // LOGIKA KRUSIAL: 
        // 1. Masukkan semua proses baru yang datang selama interval eksekusi tadi
        for (int j = 0; j < n; j++) {
            // Cek jika proses baru datang di antara startTime dan currentTime
            if (!inQueue[j] && p[j].at <= currentTime && p[j].at > startTime) {
                queue[tail++] = j;
                inQueue[j] = true;
            }
            // Khusus untuk yang datang bersamaan dengan selesainya kuantum (p[j].at == currentTime)
            // juga akan masuk ke sini sebelum proses lama dimasukkan kembali.
        }

        // 2. Jika proses belum selesai, masukkan kembali ke antrean (setelah proses baru)
        if (remainingBT[i] > 0) {
            queue[tail++] = i;
        } else {
            // Jika proses selesai
